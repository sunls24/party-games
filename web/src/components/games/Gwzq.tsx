...
function countBy(
  ...
  next: (
    i: number,
    last: boolean
  ) => { idx: number; ctIdx?: number; ct?: CellType }
) {
  let ret = 0
  for (let i = count === 0 ? 0 : 1; i <= count; i++) {
    const n = next(i, i === count)
    if (count > 0) {
      const nv = state[n.idx][ct]
      if (nv !== val) {
        break
      }
      ret++
    ...
    if (n.ct) {
      state[n.ctIdx ?? n.idx][n.ct] === val && ret++
    }
  }
  return ret
}

function nextLeft(idx: number, i: number) {
  return idx - i
}

function nextRight(idx: number, i: number) {
  return idx + i
}

function nextTop(idx: number, i: number) {
  return idx - ROWS * i
}

function nextBottom(idx: number, i: number) {
  return idx + ROWS * i
}

function rightCount(row: number) {
  return ROWS - 1 - row
}

function bottomCount(col: number) {
  return COLS - 1 - col
}
...
function checkWin(state: number[][], idx: number, ct: CellType, val: number) {
  ...
  switch (ct) {
    case CellType.Normal:
      // left
      count += countBy(state, Math.min(4, row), ct, val, (i) => ({
        idx: nextLeft(idx, i),
      }))
      ...
      count += countBy(
        state,
        Math.min(4, rightCount(row)),
        ct,
        val,
        (i, last) => ({
          idx: nextRight(idx, i),
          ct:
            last && board[nextRight(idx, i)].rightCell
              ? CellType.Right
              : CellType.Normal,
        })
      )
      ...
      // top
      count = 1
      count += countBy(state, Math.min(4, col), ct, val, (i) => ({
        idx: nextTop(idx, i),
      }))
      ...
      count += countBy(
        state,
        Math.min(4, bottomCount(col)),
        ct,
        val,
        (i, last) => ({
          idx: nextBottom(idx, i),
          ct:
            last && board[nextBottom(idx, i)].bottomCell
              ? CellType.Bottom
              : CellType.Normal,
        })
      )
      ...
      // left-top
      count = 1
      count += countBy(state, Math.min(4, row, col), ct, val, (i) => ({
        idx: nextTop(nextLeft(idx, i), i),
      }))
      console.log("check left-top", count)
      if (count >= 5) {
        break
      }
      ...
      count += countBy(
        state,
        Math.min(4, rightCount(row), bottomCount(col)),
        ct,
        val,
        (i, last) => {
          const next = nextRight(nextBottom(idx, i), i)
          if (last && board[next].rightCell) {
            return {
              idx: next,
              ctIdx: nextBottom(next, 1),
              ct: CellType.Right,
            }
          }
          if (last && board[next].bottomCell) {
            return {
              idx: next,
              ctIdx: nextRight(next, 1),
              ct: CellType.Bottom,
            }
          }
          return {
            idx: next,
            ct: last && board[next].lastCell ? CellType.Last : CellType.Normal,
          }
        }
      )
      console.log("check right-bottom", count)
      if (count >= 5) {
        break
      }
      ...
      count += countBy(
        state,
        Math.min(4, rightCount(row), col),
        ct,
        val,
        (i, last) => {
          const next = nextRight(nextTop(idx, i), i)
          if (last && board[next].rightCell) {
            return {
              idx: next,
              ctIdx: nextTop(next, 1),
              ct: CellType.Right,
            }
          }
          return {
      ...
      console.log("check right-top", count)
      if (count >= 5) {
        break
      }
      ...
      count += countBy(
        state,
        Math.min(4, row, bottomCount(col)),
        ct,
        val,
        (i, last) => {
          const next = nextLeft(nextBottom(idx, i), i)
          if (last && board[next].bottomCell) {
            return {
              idx: next,
              ctIdx: nextLeft(next, 1),
              ct: CellType.Bottom,
            }
          }
          return {
      ...
      console.log("check left-bottom", count)
      if (count >= 5) {
        break
      }
      break
    case CellType.Right:
      // left
      count += countBy(state, 4, CellType.Normal, val, (i) => ({
        idx: nextLeft(idx, i) + 1,
      }))
      console.log("check left", count)
      if (count >= 5) {
        break
      ...
      count += countBy(
        ...
        (i, last) => ({
          idx: nextBottom(idx, i),
          // TODO: right 和 bottom 都会碰到是 last 的情况，欠考虑，后面换个算法
          ct:
            last && board[nextBottom(idx, i)].bottomCell
              ? CellType.Bottom
              : CellType.Normal,
